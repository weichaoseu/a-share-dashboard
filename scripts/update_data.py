...
TARGETS={
"科创50":("index","000688"),"创业板50":("index","399673"),"沪深300":("index","000300"),
...
def hist(kind,code):
    if kind=="index":
        ...
        ren={"date":"date","close":"close","volume":"volume","amount":"amount"}
    else:
        d=ak.fund_etf_hist_em(symbol=code,period="daily",start_date="20190101",end_date="20991231",adjust="qfq")
        ren={"日期":"date","收盘":"close","成交量":"volume","成交额":"amount"}
    d=d[[c for c in ren if c in d.columns]].rename(columns=ren)
...
def factors(d):
    ...
    return dict(trend=round(trend,1),smart_money=round(float(smart),1),crowding=round(float(np.clip(crowd,0,100)),1),
      daily_ret=round((c.iloc[-1]/c.iloc[-2]-1)*100,2),
      ret20=round((c.iloc[-1]/c.iloc[-21]-1)*100,2),
      ret60=round((c.iloc[-1]/c.iloc[-61]-1)*100,2),
      ret120=round((c.iloc[-1]/c.iloc[-121]-1)*100,2))
...
for k,d in raw.items():
    z=d.assign(ds=d.date.dt.strftime("%Y-%m-%d")).set_index("ds").reindex(common)
    aligned[k]=z
